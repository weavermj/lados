...
function src_simple_recent_comments($src_count=7, $src_length=60, $pre_HTML='<li><h2>Recent Comments</h2>', $post_HTML='</li>') {
	...
	foreach ($comments as $comment) {
		$output .= "\n\t<li><div class=\"author\"><a href=\"" . get_permalink($comment->ID) . "#comment-" . $comment->comment_ID  . "\" title=\"on " . $comment->post_title . "\">" . $comment->comment_author . "</a></div><div class=\"comment\">" . strip_tags($comment->com_excerpt) . "...</div></li>";
	}
	$output .= "\n</ul>";
	$output .= $post_HTML;
